        ...
        if (MY_INDEX == 0)
        {
            ...
            case DISTANCE:
                if (msg.data[0] == DISTANCE_Z)
                {
                    ...
                    printf("M recv Z%d distance frame, current slot = %d.%d, seq = %d\n", index, info.current_time_frame, info.current_slot, msg.head.seq);
                }
                else
                {
            ...
            switch (msg.head.type)
            {
                ...
                case SCAN:
                    ...
                    else if (msg.data[0] == SCAN_CON)
                    {
                        ...
                        printf("Z%d recv M scan confirm, current slot = %d.%d, seq = %d\n", MY_INDEX, info.current_time_frame, info.current_slot, msg.head.seq);
...
#endif
                   printf("Z%d recv M data, current slot = %d.%d, seq = %d\n", MY_INDEX, info.current_time_frame, info.current_slot, msg.head.seq);       
                    break;
                default:
                    break;
            }
